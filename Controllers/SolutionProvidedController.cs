...
namespace CSLog.Controllers
{
    ...
    public class SolutionProvidedController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            ViewBag.Users = _user.GetAllComplaintUsers();//.GetAll();
            ViewBag.currentComplaintUser = 0;
            ...
            if (User.IsInRole("User"))
            {
                ...
                complaints = _vComplaint.GetTodayComplaint(user.UserId).Where(c => c.ComplaintStatus.ToUpper() != "CLOSED").ToList();
            }
            else if (User.IsInRole("Support"))
            {

                var user = _user.GetUser(User.Identity.Name);
                complaints = _vComplaint.GetTodayComplaintBySupportUser(user.UserId).Where(c => c.ComplaintStatus.ToUpper() != "CLOSED").ToList();
            }
            else
            {
                complaints = _vComplaint.GetTodayComplaint().Where(c => c.ComplaintStatus.ToUpper() != "CLOSED").ToList();
            }
        ...
        public ActionResult Resolved()
        {
            ...
            {
                var complaints = _vComplaint.GetTodayComplaintBySupportUser(UserId).Where(c => c.SolutionStatus.ToUpper() == "RESOLVED FULLY").ToList();
                return View(complaints);
            }
        ...
        public ActionResult Report()
        {
            ...
            if (User.IsInRole("Admin"))
            {
                var complaints = _vComplaint.GetTodayComplaintBySupportUser().Where(c => c.SolutionStatus.ToUpper() != "RESOLVED FULLY").ToList(); ;
                return View(complaints);
            }
            else
            {
        ...
        public ActionResult Report(DateTime sDate, DateTime eDate, int statusId, int typeId)
        {
            ...
            {
                var complaints = _vComplaint.FilterComplaintBySupportUser(sDate, eDate, typeId, statusId,0,UserId).Where(c => c.SolutionStatus.ToUpper() != "RESOLVED FULLY").ToList(); ;
                return View(complaints);
            }
            //var complaints = _vComplaint.FilterComplaint(sDate, eDate, typeId, statusId).Where(c => c.SolutionStatus.ToUpper() != "RESOLVED FULLY").ToList(); ;
            //return View(complaints);
